...
Box::Box(GLfloat edgeLength) : Box(edgeLength, edgeLength, edgeLength)
{
}
...
void Box::fill(Buffer& vertexBuffer) const
{
	std::vector<Vec3> buf;
...
void Box::fill(Buffer& vertexBuffer, Buffer& indexBuffer) const
{
	vertexBuffer.bind();
...
const std::vector<Vec3>& Box::getVertices() const
{
	return vertices;
}

const std::vector<GLushort>& Box::getIndices() const
{
	return indices;
}